...
export async function runIngestion() {
  ...
  for (const f of funds) {
    await runQuery(
      `INSERT INTO funds
         (fund_id, fund_name, category, amc, fund_manager, benchmark,
          expense_ratio, aum_cr, current_nav, nav_date, launch_date)
      ...
      [
        f.fund_id, f.fund_name, f.category, f.amc,
        f.fund_manager, f.benchmark, f.expense_ratio,
        f.aum_cr, f.current_nav, f.nav_date, f.launch_date || null,
      ]
    );
  }
  ...
  for (const h of holdings) {
    await runQuery(
      `INSERT INTO holdings
         (fund_id, fund_name, units, purchase_nav, purchase_date, folio_no)
       VALUES ($1,$2,$3,$4,$5,$6)`,
  ...
  for (const t of txns) {
    await runQuery(
      `INSERT INTO transactions
         (txn_id, fund_id, folio_no, txn_type, units, nav, amount, txn_date)
  ...
  console.log(' Ingestion complete → tara_intelligence');
}